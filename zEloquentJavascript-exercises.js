...
// 11.1 Find location of scalpel among set of nests 
//11.1.1: async function
...
//11.2 Promise_all function 

function Promise_all(promises) {
    return new Promise((resolve, reject) => {
      let counter = promises.length 
      let resolvedPromises = [] 
      if (counter == 0) {resolve(resolvedPromises)}
      for (let i = 0; i < promises.length; i++) {
         promises[i].then(value => {
         resolvedPromises[i] = value
         counter--
         if (counter === 0) {resolve(resolvedPromises)}}).catch(reject)
      }
    })
  }
